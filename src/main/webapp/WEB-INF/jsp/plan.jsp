...
$(window).resize(function() {
	$("#grid-planList").setGridWidth($(".sect-cont .line-table").width());
});

function initGridListPlan(gridData)	{
	$("#grid-planList").jqGrid('GridUnload');
	$("#grid-planList").jqGrid({
	...
	if(gridData.length == 0){
        $("#grid-planList").after("<p id='list_nodata' style='margin-top:10px;text-align:center;font-weight:bold;color:#fff;font-size:14px;'>검색 결과가 없습니다.</p>");
    }
}
/* grid에서 버튼 처리 */
